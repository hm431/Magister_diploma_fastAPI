...
from app.models.ref_object import Site
...
def _euclidean(x1: Optional[float], y1: Optional[float],
               x2: Optional[float], y2: Optional[float]) -> Optional[float]:
    """Евклидово расстояние; None если координаты отсутствуют."""
...
async def assign_brigades(
    ...
    for item in items:
        ...
        if sid not in site_data:
            site_data[sid] = {
                ...
                "required_quals": set(),
                "work_type_ids": set(),
                "total_volume": 0.0,   # суммарный объём работ на участке
            }
        d = site_data[sid]
        ...
        d["total_volume"] += float(w.volume)

    site_ids = list(site_data.keys())

    # ── 4. Загрузка участков с координатами ───────────────────────────────
    ...
    # ── 5. Загрузка бригад с квалификациями и координатами ────────────────
    ...
    brigades_map: Dict[int, Brigade] = {b.brigade_id: b for b in brigades}

    # ── 6. Нормы расхода: work_type_id → set material_id ──────────────────
    ...
    # ── 7. Множество материалов J_s на каждом участке ─────────────────────
    ...
    # ── 8. Глобальные остатки V_j (по всем складам) ───────────────────────
    mat_stock_global: Dict[int, float] = {}
    if all_mats:
        latest_q = (
            select(
        ...
        for mat_id, ld in latest_dates.items():
            ...
            mat_stock_global[mat_id] = max(0.0, float(val))

    # ── 9. Плановая потребность Q_js по участку (из MaterialDemand) ────────
    # Загружаем MaterialDemand для данного расчёта, разбитый по site через работу
    site_mat_demand: Dict[int, Dict[int, float]] = {sid: {} for sid in site_ids}
    if all_mats:
        # Q_js = суммарная потребность материала j на участке s
        # Берём из MaterialDemand.schedule_calculation_id = calculation_id
        ...
        total_demand_by_mat: Dict[int, float] = {r.material_id: float(r.total) for r in demand_rows}

        # Рассчитываем долю участка s в общей потребности материала j
        # Q_js = Q_j_total * (volume_s / volume_total) — пропорционально объёму работ
        total_volume_all = sum(d["total_volume"] for d in site_data.values()) or 1.0
        for sid in site_ids:
            share_s = site_data[sid]["total_volume"] / total_volume_all
            for mat_id in site_materials.get(sid, set()):
                q_j = total_demand_by_mat.get(mat_id, 0.0)
                site_mat_demand[sid][mat_id] = q_j * share_s

    # ── 10. Коэффициент обеспеченности m_s (формула 2.36) ─────────────────
    # m_s = (1/|J_s|) * Σ min(1, V_js / Q_js)
    # V_js = доля глобального остатка, пропорциональная потребности участка s
    m_s: Dict[int, float] = {}
    for sid in site_ids:
        ...
        total = 0.0
        for j in j_s:
            q_js = site_mat_demand[sid].get(j, 0.0)
            if q_js <= 0.0:
                total += 1.0   # материал не нужен — считаем обеспеченным
                continue
            # V_js: пропорциональная часть глобального остатка для участка s
            v_js = mat_stock_global.get(j, 0.0) * (q_js / max(
                sum(site_mat_demand[s].get(j, 0.0) for s in site_ids), 1e-9
            ))
            total += min(1.0, v_js / q_js)
        m_s[sid] = total / len(j_s)

    # ── 11. Нормировка расстояний для g_ks ────────────────────────────────
    # Вычисляем все попарные расстояния brigade → site, затем нормируем.
    # Если координаты отсутствуют — g_ks = 0 (влияние α2 = 0).
    raw_dist: Dict[Tuple[int, int], Optional[float]] = {}
    for b in brigades:
        for sid in site_ids:
            site = sites_map.get(sid)
            d = _euclidean(
                b.home_x, b.home_y,
                float(site.location_x) if site and site.location_x is not None else None,
                float(site.location_y) if site and site.location_y is not None else None,
            )
            raw_dist[(b.brigade_id, sid)] = d

    finite_dists = [v for v in raw_dist.values() if v is not None]
    max_dist = max(finite_dists) if finite_dists else 0.0

    def g_ks(b_id: int, sid: int) -> float:
        """Нормированное расстояние ∈ [0, 1]. 0 если координаты не заданы."""
        d = raw_dist.get((b_id, sid))
        if d is None or max_dist == 0.0:
            return 0.0
        return d / max_dist

    # ── 12. Матрица стоимостей c_ks (формула 2.35) ────────────────────────
    ...
    for ki, b_id in enumerate(brigade_ids):
        ...
        for si, s_id in enumerate(site_ids):
            ...
            cost[ki, si] = (
                params.alpha1 * h_ks
                + params.alpha2 * g_ks(b_id, s_id)
                + params.alpha3 * (1.0 - m_s[s_id])
            )

    # ── 13. Расширение матрицы (до n_max бригад на участок) ───────────────
    n_max = params.n_max
    cost_exp = np.tile(cost, n_max)   # nK × (nS * n_max)
    nS_exp = nS * n_max
    ...
    # ── 14. Венгерский алгоритм (формула 2.37) ────────────────────────────
    row_ind, col_ind = linear_sum_assignment(cost_sq)

    # ── 15. Удаление предыдущих назначений ────────────────────────────────
